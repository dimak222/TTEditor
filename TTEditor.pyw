                    ...
                    try: # попытаться заменить файлы
                        ...
                        if os.path.exists(file_full_name): # если есть файл в папке с программой
                            ...
                            if filecmp.cmp(file_server_path, file_full_name, shallow = True) == False: # если файлы разные, обработать (сравнивает только метаданные файлов)
                                ...
                                self.message(f"Файл \"{file_name}\" обновлён!") # сообщение, поверх всех окон с автоматическим закрытием

                        else: # нет файла в папке с программой
                            shutil.copy2(file_server_path, file_full_name) # копируем файл с сервера с сохранением методанных
                            self.message(f"Файл \"{file_name}\" обновлён с сервера!", 2) # сообщение, поверх всех окон с автоматическим закрытием

                    except: # в случае ошибки
                        self.message(f"Нет прав на замену файла \"{file_name}\".\nЗапустите программу от администратора\nили переместите её в другую папку!") # сообщение, поверх всех окон с автоматическим закрытием

                else: # нет файла на сервере
                    if msg_server: # выдавать сообщения если на сервере нет .txt файлов
                        self.message(f"Файл \"{file_name}\" на сервере не найден!") # сообщение, поверх всех окон с автоматическим закрытием
